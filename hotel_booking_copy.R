...
#1:looking at the summary, the children column has 4 NA cases, these 4 NA cases constitute an unsignificant percentage in the whole dataset of 0.003%, hence, the team verdict to remove the 4 cases with NA value.
...
##Now, we will have data visualization of our dataset--------------------------
...
#Class balance of Target Variable 33098 cancellations compared to 46226 non-cancellations
...
log1 <- glm(is_canceled ~.,family = binomial, data = hb3.dt)
summary(log1)
...
x<-model.matrix(is_canceled~.,data=hb3.dt)
x=x[,-1]
lasso<-cv.glmnet(x=x,y=hb3.dt$is_canceled,type.measure='mse',nfolds=5,alpha=.5,family="binomial")
c<-coef(lasso,s='lambda.min',exact=TRUE)
inds<-which(c==0)
variables<-row.names(c)[inds]
variables
#insignificant variables in lasso regression
#we will not be putting in reservation_status as it is ultimately showing the cancellation rate of the hotel. Thus, will not give us insightful results
hb3.dt<-hb3.dt[,!c("country","reservation_status")]
...
#log model
log1<- glm(is_canceled ~ .,family = binomial, data = hb3_trainset)
summary(log1)
log2 <- glm (is_canceled ~ .-required_car_parking_spaces-is_repeated_guest, family = binomial, data = hb3_trainset)
summary (log2)

# Confusion Matrix on Testset
log2.test <- predict(log2, newdata = hb3_testset, type = 'response')
log2.predict <- ifelse(log2.test > threshold1, "1", "0")
table1 <- table(Testset.Actual = hb3_testset$is_canceled, log2.predict, deparse.level = 2)
table1
...
table3 <- table(Testset.Actual = hb3_testset$is_canceled, m.RF.1.test.prob, deparse.level = 2)
table3
