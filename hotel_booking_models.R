...
cat("Trainset AUC-ROC score is:", auc_roc) # Trainset AUC-ROC score is: 0.858566
test_roc <- roc(hb3_testset$is_canceled, log2.test)
auc_roc <- auc(test_roc)
cat("Testset AUC-ROC score is:", auc_roc) # Testset AUC-ROC score is: 0.8586535
...
table1 <- table(Trainset.Actual = hb3_trainset$is_canceled, RF.train, deparse.level = 2)
table1 
...
table2 <- table(Testset.Actual = hb3_testset$is_canceled, m.RF.1.test.prob, deparse.level = 2)
table2
...
table2 <- table(Testset.Actual = hb3_testset$is_canceled,test_preds, deparse.level = 2)
table2
round(prop.table(table3), 3)
mean(m.RF.1.test.prob == hb3_testset$is_canceled)

# RANDOM FOREST MODEL WITH ADJUSTED MTRY =======================================
m.RF.1<-randomForest(is_canceled~.,data=hb3_trainset,importance=T, cv.fold=10, mtry=20)
m.RF.1
var.impt2 <- importance(m.RF.1)
var.impt2
varImpPlot(m.RF.1, type = 1)
...
train_roc <- roc(hb3_trainset$is_canceled, train_pred)
auc_roc <- auc(train_roc)
cat("Trainset AUC-ROC score is:", auc_roc) #0.9291

test_pred <- predict(m.RF.1, newdata = hb3_testset, type = "prob")[,2]
test_roc <- roc(hb3_testset$is_canceled, test_pred)
auc_roc <- auc(test_roc)
cat("Testset AUC-ROC score is:", auc_roc) #0.9272
