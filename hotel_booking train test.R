...
#Class balance of Target Variable 31213 cancellations compared to 42543 non-cancellations
...
x=x[,-1]
lasso<-cv.glmnet(x=x,y=hb3_trainset$is_canceled,type.measure='mse',nfolds=5,alpha=.5,family="binomial")
...
summary(log1)
log2 <- glm (is_canceled ~ .-required_car_parking_spaces-is_repeated_guest-market_segment-days_in_waiting_list, family = binomial, data = hb3_trainset)
summary (log2)

# Confusion Matrix on Trainset
log2.train <- predict(log2, type = "response")
log2.predict.train <- ifelse(log2.train > threshold1, "1", "0")
